...
const ContainerModifier = {
  ...
  ignorePadding: (theme: DefaultTheme, confirm?: boolean) => css`
    padding: ${() => {
      return confirm
        ? `${theme.padding.XLg} ${theme.padding.XLg} 0`
        : theme.padding.XLg;
    }};
  `,
  ignoreRadius: () => css`
...
export const Modal = styled.div<ModalProps>`
  ...
  ${({ transparent, theme }) =>
    !transparent && ContainerModifier.transparent(theme)}
  ${({ ignoreRadius }) => ignoreRadius && ContainerModifier.ignoreRadius()}
  ${centerAbsolute};
`;

export const Content = styled.div<ModalProps>`
  ${({ theme, ignorePadding, maxWidth, maxHeight, height, confirm }) => css`
    display: flex;
...
export const ButtonContainer = styled.div`
  padding: ${({ theme }) =>
    `${theme.padding.Sm} ${theme.padding.XLg} ${theme.padding.Lg}`};
`;
